...
import { ArrowLeft, Check, Loader2, Star, RefreshCw } from "lucide-react";
import { toast } from "sonner";
import { api, formatINR, type CatalogModule, type Package } from "@/lib/api";

export const Route = createFileRoute("/admin/_shell/packages/$id/edit")({
  head: () => ({ meta: [{ title: "Edit Package — MediOps" }] }),
...
function EditPackage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    setLoadError(null);
    Promise.all([api.packages.get(packageId), api.catalog.modules()])
      .then(([p, mods]) => {
        setPkg(p);
        setForm({
          name: p.name,
        ...
        setModules(mods);
        setSelected(p.modules?.map((m) => m.module.id) ?? []);
      })
      .catch((e: unknown) =>
        setLoadError(e instanceof Error ? e.message : "Failed to load package"),
      )
      .finally(() => {
        setLoading(false);
  ...
  function validate(current: FormState): boolean {
    ...
    if (!current.name.trim()) e.name = "Package name required";
  ...
  async function submit() {
    if (!form || !pkg || !validate(form)) return;
    setSubmitting(true);
    try {
      ...
      // Sync modules: attach what's newly picked, detach what was removed.
      const original = new Set(pkg.modules?.map((m) => m.module.id) ?? []);
      const next = new Set(selected);
      const toAttach = selected.filter((id) => !original.has(id));
      const toDetach = [...original].filter((id) => !next.has(id));

      for (const moduleId of toAttach) await api.packages.attachModule(pkg.id, moduleId);
      for (const moduleId of toDetach) await api.packages.detachModule(pkg.id, moduleId);

      toast.success(`${form.name.trim().toUpperCase()} package updated`);
  ...
  if (loadError || !form || !pkg) {
    return (
      <div className="text-center py-20">
        <div className="font-display text-xl font-bold">Failed to load package</div>
        {loadError && <div className="text-xs text-muted-foreground mt-1">{loadError}</div>}
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="font-display text-2xl font-bold">Edit {form.name || "package"}</h1>
        <p className="text-sm text-muted-foreground">
          Update pricing, limits and module access. Changes apply to newly billed cycles.
        </p>
      </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Field label="Monthly price ₹" error={errors.monthlyPrice}>
              <NumberInput
                ...
                onChange={(n) => setForm((f) => (f ? { ...f, monthlyPrice: n } : f))}
              />
            </Field>
            <Field label="Yearly price ₹" error={errors.yearlyPrice}>
              <NumberInput
                value={form.yearlyPrice}
                onChange={(n) => setForm((f) => (f ? { ...f, yearlyPrice: n } : f))}
              />
            </Field>
